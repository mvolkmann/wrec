...
test("sanitize preserves comments alongside Wrec-style event handlers", () => {
  const value = sanitize('<!-- this.count --><button onclick="increment" class="ok">Save</button>');
...
test("sanitize restores comments even when placeholder-like text is present", () => {
  const value = sanitize('<div>__WREC0__</div><!-- this.name --><span class="ok">Hello</span>');
