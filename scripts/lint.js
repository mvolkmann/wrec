...
function addExpression(expressions, text, node, kind) {
  const trimmed = text.trim();
  if (!trimmed) return;
  const location = node
    .getSourceFile()
    .getLineAndCharacterOfPosition(node.getStart());
  expressions.push({context: 'instance', kind, text: trimmed, location});
}
...
function main() {
  ...
  if (findings.incompatibleArguments.length > 0) {
    ...
    findings.incompatibleArguments.forEach(finding => {
      console.log(
        `  ${finding.methodName}: argument "${finding.argument}" is ${finding.argumentType}, parameter "${finding.parameterName}" expects ${finding.parameterType}`
      );
    });
  }
