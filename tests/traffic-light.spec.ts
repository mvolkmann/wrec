...
async function expectState(component: Locator, state: string) {
  ...
  await expect(component.locator(".stop")).toHaveClass(state === "stop" ? "stop on" : "stop");
  await expect(component.locator(".yield")).toHaveClass(state === "yield" ? "yield on" : "yield");
  await expect(component.locator(".go")).toHaveClass(state === "go" ? "go on" : "go");
}
...
test("traffic-light next method advances state", async ({ page }: { page: Page }) => {
  const component = page.locator("traffic-light");
...
test("traffic-light state property can be set", async ({ page }: { page: Page }) => {
  const component = page.locator("traffic-light");
